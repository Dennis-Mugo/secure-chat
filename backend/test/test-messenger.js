...
const govDecrypt = async function (secret, [header, ct]) {
  ...
  const subtleMK = await subtle.importKey("raw", mk, "AES-GCM", true, [
    "encrypt",
    "decrypt",
  ]);
  const plaintext = await decryptWithGCM(subtleMK, ct, header.receiverIV);
...
describe("Messenger", function () {
  ...
  describe("functionality", function () {
    ...
    it("imports a certificate without an error", async function () {
      ...
      const bobCertSignature = await signWithECDSA(
        caKeyPair.sec,
    ...
    it("bob can recieve an encrypted message from alice", async function () {
      ...
      const bobCertificate = await bob.generateCertificate("bob");
      const aliceCertSignature = await signWithECDSA(
        caKeyPair.sec,
    ...
    it("certificates with invalid signatures are rejected", async function () {
      ...
      const invalidSignature = await signWithECDSA(
        caKeyPair.sec,
        "fake_signature"
      );
      await expect(alice.receiveCertificate(bobCertificate, bobCertSignature))
        .to.not.be.rejected;
      await expect(alice.receiveCertificate(bobCertificate, invalidSignature))
        .to.be.rejected;
    });
    ...
    it("alice rejects messages where she is not intended recipient", async function () {
      ...
      const claireCertSignature = await signWithECDSA(
        caKeyPair.sec,
        stringifyCert(claireCertificate)
      );
      await alice.receiveCertificate(bobCertificate, bobCertSignature);
    ...
    it("alice can have simultaneous and separate conversations with Bob, Claire, and Dave", async function () {
      ...
      const daveCertSignature = await signWithECDSA(
        caKeyPair.sec,
        stringifyCert(daveCertificate)
      );
